...
void sendIotMessage(char* msg) {
  ...
  Esp32MQTTClient_Event_AddProp(message, "DeviceId", "40:EC:99:EC:28:0E");
  Esp32MQTTClient_Event_AddProp(message, "Longitude", "17.912470");
  Esp32MQTTClient_Event_AddProp(message, "Latitude", "59.411518");
  Esp32MQTTClient_Event_AddProp(message, "SensorType", "DHT-11");
  Esp32MQTTClient_Event_AddProp(message, "Vendor", "Adafruit");
  Esp32MQTTClient_Event_AddProp(message, "Model", "ESP32 Feather Huzzah");
...
void loop() {
  ...
  if (difference()) {
    ...
    if(temp < 23 || temp > 26)
      root["TemperatureAlert"] = false;
     else
      root["TemperatureAlert"] = true; 
    root["Humidity"] = hum;
